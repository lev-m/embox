...
#define PAGE_MASK               (~(XEN_PAGE_SIZE-1))
#define PADDR_BITS              44
#define PADDR_MASK              ((1ULL << PADDR_BITS)-1)

#define to_virt(x)              ((void *)((unsigned long)(x)+VIRT_START))
#define pte_to_mfn(_pte)        (((_pte) & (PADDR_MASK&PAGE_MASK)) >> L1_PAGETABLE_SHIFT)
#define pte_to_virt(_pte)       to_virt(mfn_to_pfn(pte_to_mfn(_pte)) << XEN_PAGE_SHIFT)
#define mfn_to_virt(_mfn)       (to_virt(mfn_to_pfn(_mfn) << XEN_PAGE_SHIFT))
#define pfn_to_virt(_pfn)       (to_virt((_pfn) << XEN_PAGE_SHIFT))
#define _virt_to_mfn(_virt)     (pfn_to_mfn(virt_to_pfn(_virt)))
#define MAX_MEM_SIZE            0x3f000000ULL

#define PFN_UP(x)	(((x) + XEN_PAGE_SIZE-1) >> L1_PAGETABLE_SHIFT)
...
static void new_pt_frame(unsigned long *pt_pfn, unsigned long prev_l_mfn, 
	...
	memset((void*) pt_page, 0, XEN_PAGE_SIZE);  
	...
	mmu_updates[0].val = (pgentry_t)pfn_to_mfn(*pt_pfn) << XEN_PAGE_SHIFT | 
	...
	mmu_updates[0].ptr =
		((pgentry_t)prev_l_mfn << XEN_PAGE_SHIFT) + sizeof(pgentry_t) * offset;
	mmu_updates[0].val = (pgentry_t)pfn_to_mfn(*pt_pfn) << XEN_PAGE_SHIFT |
		pt_prot[level];
...
static void build_pagetable(unsigned long *start_pfn, unsigned long *max_pfn) {
	...
	if ( *max_pfn >= virt_to_pfn(HYPERVISOR_VIRT_START) )
	{
		...
		*max_pfn = virt_to_pfn(HYPERVISOR_VIRT_START - XEN_PAGE_SIZE);
	...
	while ( start_address < end_address )
	{
		...
		tab = to_virt(mfn_to_pfn(pt_mfn) << XEN_PAGE_SHIFT);
		...
		if ( !(tab[offset] & _PAGE_PRESENT) )
		{
			mmu_updates[count].ptr =
				((pgentry_t)pt_mfn << XEN_PAGE_SHIFT) + sizeof(pgentry_t) * offset;
			mmu_updates[count].val =
				(pgentry_t)pfn_to_mfn(pfn_to_map) << XEN_PAGE_SHIFT | L1_PROT;
			count++;
		}
		pfn_to_map++;
...
void arch_init_mm(unsigned long* start_pfn_p, unsigned long* max_pfn_p) {
	...
	if ( max_pfn >= MAX_MEM_SIZE / XEN_PAGE_SIZE )
	{
		max_pfn = MAX_MEM_SIZE / XEN_PAGE_SIZE - 1;
	}
...
static int memory_init(char *const xen_mem_alloc_start, char *const xen_mem_alloc_end) {
	...
	if (va) {
		__xen_mem_allocator = page_allocator_init(xen_mem_alloc_start, mem_len, XEN_PAGE_SIZE);
	}
